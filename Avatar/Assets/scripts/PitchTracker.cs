...
public class PitchTracker : MonoBehaviour
{
    ...
    public int binSize = 10; // you can change this up, I originally used 8192 for better resolution, but I stuck with 1024 because it was slow-performing on the phone
    public float refValue = 0.1f;
    public float threshold = Mathf.Pow(10, -15);
    ...
    void Update()
    {
        if (stoptime == 0 || currenttime < stoptime)
        {
            ...
            if (getAvg && pitchValue > 0)
            {
                ...
                {
                    avgPitch = (avgPitch + pitchValue) / 2f;
                }
